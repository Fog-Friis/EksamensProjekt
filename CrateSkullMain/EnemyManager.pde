class EnemyManager {
  ...
  void enemyCollision(ArrayList<Enemy> e, ArrayList<ShooterEnemy> s){
    ...
    for (int i = 0; i < e.size(); i++) {
      for (int j = 0; j < s.size(); j++) {      
          if (dist(e.get(i).pos.x, e.get(i).pos.y, s.get(j).pos.x, s.get(j).pos.y) <= 45) {
            float theta = atan2(e.get(i).pos.y-s.get(j).pos.y, e.get(i).pos.x-s.get(j).pos.x)+PI;
            e.get(i).pos = e.get(i).pos.add(new PVector(45*cos(theta),45*sin(theta)));
            s.get(j).pos = s.get(j).pos.sub(new PVector(45*cos(theta),45*sin(theta)));
        }
      }
    }
  }
